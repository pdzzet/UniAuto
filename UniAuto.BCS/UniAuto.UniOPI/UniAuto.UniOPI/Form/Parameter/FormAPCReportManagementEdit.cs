...
namespace UniOPI
{
    public partial class FormAPCReportManagementEdit : FormParamManagementBaseEdit
    {
        private SBRM_APCDATAREPORT ObjEdit;

        public FormAPCReportManagementEdit()
            : base(ParamForm.APCReportManagement)
        {
            InitializeComponent();
        }

        public FormAPCReportManagementEdit(SBRM_APCDATAREPORT objData)
            : this()
        {
            if (objData == null)
            {
        ...
        private void FormAPCReportManagementEdit_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            try
            {
                ...
                switch (FormMode)
                {
                    case FormMode.AddNew:
                        ...
                        SBRM_APCDATAREPORT _objData = null;

                        foreach (DataGridViewRow _row in dgvAddList.Rows)
                        {
                            _objData = new SBRM_APCDATAREPORT();
                            ...
                            _ctxBRM.SBRM_APCDATAREPORT.InsertOnSubmit(_objData);
                        }
