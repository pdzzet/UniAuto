...
namespace UniOPI
{
    public partial class FormProcessDataManagementEdit : FormParamManagementBaseEdit
    {
        private SBRM_PROCESSDATA ObjEdit;

        public FormProcessDataManagementEdit()
            : base(ParamForm.ProcessDataManagement)
        {
            InitializeComponent();
        }

        public FormProcessDataManagementEdit(SBRM_PROCESSDATA objData)
            : this()
        {
            if (objData == null)
            {
        ...
        private void FormProcessDataManagementEdit_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            try
            {
                ...
                switch (FormMode)
                {
                    case FormMode.AddNew:
                        ...
                        SBRM_PROCESSDATA _objData = null;

                        foreach (DataGridViewRow _row in dgvAddList.Rows)
                        {
                            _objData = new SBRM_PROCESSDATA();
                            ...
                            _ctxBRM.SBRM_PROCESSDATA.InsertOnSubmit(_objData);
                        }
