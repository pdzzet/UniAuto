...
namespace UniOPI
{
    public partial class FormProcessDataManagement : FormParamManagementBase
    {
        public FormProcessDataManagement()
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                FormProcessDataManagementEdit frmAdd = new FormProcessDataManagementEdit(null);
        ...
        private void btnModify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SBRM_PROCESSDATA objSelModify = (dgvData.SelectedRows[0].DataBoundItem) as SBRM_PROCESSDATA;
                UniBCSDataContext ctxBRM = FormMainMDI.G_OPIAp.DBBRMCtx;
                SBRM_PROCESSDATA objProcess = null;

                if (objSelModify.OBJECTKEY == 0)
                {
                    // 修改的是尚未Submit的新增
                    var objAddModify = (from apc in ctxBRM.GetChangeSet().Inserts.OfType<SBRM_PROCESSDATA>()
                ...
                FormProcessDataManagementEdit frmModify = new FormProcessDataManagementEdit(objProcess);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                UniBCSDataContext ctxBRM = FormMainMDI.G_OPIAp.DBBRMCtx;
                SBRM_PROCESSDATA objEach = null, objToDelete = null;

                foreach (DataGridViewRow selectedRow in dgvData.SelectedRows)
                {
                    objEach = selectedRow.DataBoundItem as SBRM_PROCESSDATA;
                    if (objEach.OBJECTKEY > 0)
                    {
                        ctxBRM.SBRM_PROCESSDATA.DeleteOnSubmit(objEach);
                    }
                    else
                    {
                        objToDelete = (from proc in ctxBRM.GetChangeSet().Inserts.OfType<SBRM_PROCESSDATA>()
                        ...
                        if (objToDelete != null)
                            ctxBRM.SBRM_PROCESSDATA.DeleteOnSubmit(objToDelete);
                    }
                }
        ...
        private void dgvData_DataSourceChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    SBRM_PROCESSDATA obj = dr.DataBoundItem as SBRM_PROCESSDATA;

                    if (ctxBRM.GetChangeSet().Inserts.OfType<SBRM_PROCESSDATA>().Any(
                    ...
                    else if (ctxBRM.GetChangeSet().Deletes.OfType<SBRM_PROCESSDATA>().Any(
                        msg => msg.LINEID == obj.LINEID && msg.LINETYPE == obj.LINETYPE &&
                        msg.NODENO == obj.NODENO && msg.PARAMETERNAME == obj.PARAMETERNAME))
                    {
                        dr.DefaultCellStyle.BackColor = Color.Red;
                    }
                    else if (ctxBRM.GetChangeSet().Updates.OfType<SBRM_PROCESSDATA>().Any(
                        msg => msg.LINEID == obj.LINEID && msg.LINETYPE == obj.LINETYPE &&
                        msg.NODENO == obj.NODENO && msg.PARAMETERNAME == obj.PARAMETERNAME))
                    {
        ...
        protected override void GetGridViewData(bool showMessage = false)
        {
            try
            {
                ...
                //已修改未更新物件
                var addData = ctxBRM.GetChangeSet().Inserts.OfType<SBRM_PROCESSDATA>().Where(
                    msg => msg.SERVERNAME == FormMainMDI.G_OPIAp.CurLine.ServerName);

                List<SBRM_PROCESSDATA> objTables = selData.ToList();
        ...
        protected override void Save()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        if (objToInsert.GetType().Name != "SBRM_PROCESSDATA") continue;

                        SBRM_PROCESSDATA _updateData = (SBRM_PROCESSDATA)objToInsert;
                    ...
                    {
                        if (objToDelete.GetType().Name != "SBRM_PROCESSDATA") continue;

                        SBRM_PROCESSDATA _updateData = (SBRM_PROCESSDATA)objToDelete;
                    ...
                    {
                        if (objToUpdate.GetType().Name != "SBRM_PROCESSDATA") continue;

                        SBRM_PROCESSDATA _updateData = (SBRM_PROCESSDATA)objToUpdate;
                ...
                string _err = UniTools.InsertOPIHistory_DB("SBRM_PROCESSDATA", _sqlDesc, _sqlErr);
                ...
                Public.SendDatabaseReloadRequest("SBRM_PROCESSDATA");

                if (_sqlErr == string.Empty)
                {
                    ShowMessage(this, lblCaption.Text, "", "Process Data Save Success！", MessageBoxIcon.Information);
                }
