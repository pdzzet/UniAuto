...
namespace UniOPI
{
    public partial class FormAPCDownloadManagementEdit : FormParamManagementBaseEdit
    {
        private SBRM_APCDATADOWNLOAD ObjEdit;

        public FormAPCDownloadManagementEdit()
            : base(ParamForm.APCDownlaodManagement)
        {
            InitializeComponent();
        }

        public FormAPCDownloadManagementEdit(SBRM_APCDATADOWNLOAD objData)
            : this()
        {
            if (objData == null)
            {
            ...
            else
            {
                ...
                Size = new Size(851, 440);
            }
        }

        private void FormAPCDownloadManagementEdit_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            try
            {
                ...
                switch (FormMode)
                {
                    case FormMode.AddNew:
                        ...
                        SBRM_APCDATADOWNLOAD _objData = null;

                        foreach (DataGridViewRow _row in dgvAddList.Rows)
                        {
                            _objData = new SBRM_APCDATADOWNLOAD();
                            ...
                            _ctxBRM.SBRM_APCDATADOWNLOAD.InsertOnSubmit(_objData);
                        }
