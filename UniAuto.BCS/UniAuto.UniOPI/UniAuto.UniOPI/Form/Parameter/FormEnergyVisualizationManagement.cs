...
namespace UniOPI
{
    public partial class FormEnergyVisualizationManagement : FormParamManagementBase
    {
        public FormEnergyVisualizationManagement()
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                FormEnergyVisualizationManagementEdit frmAdd = new FormEnergyVisualizationManagementEdit(null);
        ...
        private void btnModify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SBRM_ENERGYVISUALIZATIONDATA objSelModify = (dgvData.SelectedRows[0].DataBoundItem) as SBRM_ENERGYVISUALIZATIONDATA;
                UniBCSDataContext ctxBRM = FormMainMDI.G_OPIAp.DBBRMCtx;
                SBRM_ENERGYVISUALIZATIONDATA objEnergyVis = null;

                if (objSelModify.OBJECTKEY == 0)
                {
                    // 修改的是尚未Submit的新增
                    var objAddModify = (from vis in ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ENERGYVISUALIZATIONDATA>()
                                        where vis.LINEID == objSelModify.LINEID && vis.LINETYPE == objSelModify.LINETYPE &&
                                        vis.NODENO == objSelModify.NODENO && vis.PARAMETERNAME == objSelModify.PARAMETERNAME
                                        select vis).FirstOrDefault();
                    if (objAddModify != null)
                    {
                        objEnergyVis = objAddModify;
                    }
                    else return;
                }
                else
                {
                    objEnergyVis = objSelModify;
                }

                FormEnergyVisualizationManagementEdit frmModify = new FormEnergyVisualizationManagementEdit(objEnergyVis);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                UniBCSDataContext ctxBRM = FormMainMDI.G_OPIAp.DBBRMCtx;
                SBRM_ENERGYVISUALIZATIONDATA objEach = null, objToDelete = null;
                foreach (DataGridViewRow selectedRow in dgvData.SelectedRows)
                {
                    objEach = selectedRow.DataBoundItem as SBRM_ENERGYVISUALIZATIONDATA;
                    if (objEach.OBJECTKEY > 0)
                    {
                        ctxBRM.SBRM_ENERGYVISUALIZATIONDATA.DeleteOnSubmit(objEach);
                    }
                    else
                    {
                        objToDelete = (from apc in ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ENERGYVISUALIZATIONDATA>()
                        ...
                        if (objToDelete != null)
                            ctxBRM.SBRM_ENERGYVISUALIZATIONDATA.DeleteOnSubmit(objToDelete);
                    }
                }
        ...
        private void dgvData_DataSourceChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    SBRM_ENERGYVISUALIZATIONDATA obj = dr.DataBoundItem as SBRM_ENERGYVISUALIZATIONDATA;

                    if (ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ENERGYVISUALIZATIONDATA>().Any(
                    ...
                    else if (ctxBRM.GetChangeSet().Deletes.OfType<SBRM_ENERGYVISUALIZATIONDATA>().Any(
                        msg => msg.LINEID == obj.LINEID && msg.LINETYPE == obj.LINETYPE &&
                        msg.NODENO == obj.NODENO && msg.PARAMETERNAME == obj.PARAMETERNAME))
                    {
                        dr.DefaultCellStyle.BackColor = Color.Red;
                    }
                    else if (ctxBRM.GetChangeSet().Updates.OfType<SBRM_ENERGYVISUALIZATIONDATA>().Any(
                        msg => msg.LINEID == obj.LINEID && msg.LINETYPE == obj.LINETYPE &&
                        msg.NODENO == obj.NODENO && msg.PARAMETERNAME == obj.PARAMETERNAME))
                    {
        ...
        protected override void GetGridViewData(bool showMessage = false)
        {
            try
            {
                ...
                //已修改未更新物件
                var addData = ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ENERGYVISUALIZATIONDATA>().Where(
        ...
        protected override void Save()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        if (objToInsert.GetType().Name != "SBRM_ENERGYVISUALIZATIONDATA") continue;

                        SBRM_ENERGYVISUALIZATIONDATA _updateData = (SBRM_ENERGYVISUALIZATIONDATA)objToInsert;
                    ...
                    {
                        if (objToDelete.GetType().Name != "SBRM_ENERGYVISUALIZATIONDATA") continue;

                        SBRM_ENERGYVISUALIZATIONDATA _updateData = (SBRM_ENERGYVISUALIZATIONDATA)objToDelete;
                    ...
                    {
                        if (objToUpdate.GetType().Name != "SBRM_ENERGYVISUALIZATIONDATA") continue;

                        SBRM_ENERGYVISUALIZATIONDATA _updateData = (SBRM_ENERGYVISUALIZATIONDATA)objToUpdate;
                ...
                string _err = UniTools.InsertOPIHistory_DB("SBRM_ENERGYVISUALIZATIONDATA", _sqlDesc, _sqlErr);
                ...
                Public.SendDatabaseReloadRequest("SBRM_ENERGYVISUALIZATIONDATA");

                if (_sqlErr == string.Empty)
                {
                    ShowMessage(this, lblCaption.Text, "", "Energy Visualization Data Save Success！", MessageBoxIcon.Information);
                }
