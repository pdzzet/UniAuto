...
namespace UniOPI
{
    public partial class FormRobot_Route_Step_Jump : FormBase
    {
        ...
        public FormRobot_Route_Step_Jump()
        {
            InitializeComponent();

            this.lblCaption.Text = "Robot Step Jump";
        }

        public FormRobot_Route_Step_Jump(string routeID, int stepID, Robot robot, List<int> lstStepID)
            : this()
        {
            CurRouteID = routeID;
            CurStepID = stepID;
        ...
        private void FormRobot_Route_Step_Jump_Load(object sender, EventArgs e)
        {
        ...
        private void GetDataGridView()
        {
            ...
            try
            {
                //objTables = new List<SBRM_ROBOT_RULE_ROUTESTEP_JUMP>();
                UniBCSDataContext _ctxBRM = FormMainMDI.G_OPIAp.DBBRMCtx;

                //資料庫資料
                var _selData = from rb in _ctxBRM.SBRM_ROBOT_RULE_ROUTESTEP_JUMP
                ...
                //已修改未更新物件
                var _addData = _ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_ROUTESTEP_JUMP>().Where( add => 
                    add.SERVERNAME == CurRobot.ServerName &&
                    add.ROBOTNAME == CurRobot.RobotName &&
                    add.ROUTEID == CurRouteID &&
                    add.STEPID == CurStepID);

                List<SBRM_ROBOT_RULE_ROUTESTEP_JUMP> _objTables = _selData.ToList();

                _objTables.AddRange(_addData.ToList());

                List<clsRobotRuleRouteStepJumpTable> _lstCondition = new List<clsRobotRuleRouteStepJumpTable>();

                #region Get Description & Function Key
                foreach (SBRM_ROBOT_RULE_ROUTESTEP_JUMP _data in _objTables)
                {
                    clsRobotRuleRouteStepJumpTable _tmp = new clsRobotRuleRouteStepJumpTable();
                    ...
                    _tmp.JUMPCONDITIONID = _data.JUMPCONDITIONID;
                    _tmp.JUMPITEMSEQ = _data.JUMPITEMSEQ;
                ...
                SortableBindingList<clsRobotRuleRouteStepJumpTable> _fieldList = new SortableBindingList<clsRobotRuleRouteStepJumpTable>(_lstCondition);
            ...
            try
            {
                ...
                switch (_btn.Tag.ToString())
                {
                    case "Add":
                        ...
                        #region Add
                        FormRobot_Route_Step_Jump_Edit _frmAdd = new FormRobot_Route_Step_Jump_Edit(
                    ...
                    case "Modify":
                        ...
                        clsRobotRuleRouteStepJumpTable objSelModify = (dgvData.SelectedRows[0].DataBoundItem) as clsRobotRuleRouteStepJumpTable;

                        SBRM_ROBOT_RULE_ROUTESTEP_JUMP objProcess = null;

                        if (objSelModify.OBJECTKEY == 0)
                        {
                            // 修改的是尚未Submit的新增 -- //UK : ServerName + Robot Name + RouteID + Step ID +  + Item ID     
                            var objAddModify = (from d in _ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_ROUTESTEP_JUMP>()
                                                where d.SERVERNAME == objSelModify.SERVERNAME && d.ROBOTNAME == objSelModify.ROBOTNAME && d.ROUTEID == objSelModify.ROUTEID &&
                                                      d.STEPID == objSelModify.STEPID && d.JUMPCONDITIONID == objSelModify.JUMPCONDITIONID
                        ...
                        {
                            // 修改的是尚未Submit的新增
                            var objModify = (from d in _ctxBRM.SBRM_ROBOT_RULE_ROUTESTEP_JUMP
                        ...
                        FormRobot_Route_Step_Jump_Edit _frmModify = new FormRobot_Route_Step_Jump_Edit(
                    ...
                    case "Delete":
                        ...
                        clsRobotRuleRouteStepJumpTable objEach = null;

                        SBRM_ROBOT_RULE_ROUTESTEP_JUMP objtoDelete = null;
                            
                        foreach (DataGridViewRow selectedRow in dgvData.SelectedRows)
                        {
                            objEach = selectedRow.DataBoundItem as clsRobotRuleRouteStepJumpTable;

                            if (objEach.OBJECTKEY > 0)
                            {
                                objtoDelete = (from proc in _ctxBRM.SBRM_ROBOT_RULE_ROUTESTEP_JUMP
                            ...
                            {
                                objtoDelete = (from proc in _ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_ROUTESTEP_JUMP>()
                                               where proc.SERVERNAME.Equals(objEach.SERVERNAME) &&
                                                     ...
                                                     proc.JUMPCONDITIONID.Equals(objEach.JUMPCONDITIONID)
                                               select proc).FirstOrDefault();
                            }

                            if (objtoDelete != null)
                                _ctxBRM.SBRM_ROBOT_RULE_ROUTESTEP_JUMP.DeleteOnSubmit(objtoDelete);
                        }
                    ...
                    case "Save":
                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    if (objToInsert.GetType().Name != "SBRM_ROBOT_RULE_ROUTESTEP_JUMP") continue;
                                    SBRM_ROBOT_RULE_ROUTESTEP_JUMP _updateData = (SBRM_ROBOT_RULE_ROUTESTEP_JUMP)objToInsert;

                                    _updateData.LASTUPDATETIME = _upDateTime;
                                    _sqlDesc = _sqlDesc + (_sqlDesc == string.Empty ? string.Empty : ";") + string.Format("ADD [{0},{1},{2},{3},{4},{5},{6},{7}]", _updateData.ROBOTNAME, _updateData.ROUTEID, _updateData.STEPID, _updateData.JUMPCONDITIONID, _updateData.GOTOSTEPID, _updateData.JUMPITEMSEQ, _updateData.ISENABLED, _updateData.DESCRIPTION);
                                }
                                #endregion
                                ...
                                foreach (object objToModify in _ctxBRM.GetChangeSet().Updates)
                                {
                                    ...
                                    SBRM_ROBOT_RULE_ROUTESTEP_JUMP _updateData = (SBRM_ROBOT_RULE_ROUTESTEP_JUMP)objToModify;
                                ...
                                {
                                    if (objToDelete.GetType().Name != "SBRM_ROBOT_RULE_ROUTESTEP_JUMP") continue;
                                    SBRM_ROBOT_RULE_ROUTESTEP_JUMP _updateData = (SBRM_ROBOT_RULE_ROUTESTEP_JUMP)objToDelete;
                            ...
                            string _err = UniTools.InsertOPIHistory_DB("SBRM_ROBOT_RULE_ROUTESTEP_JUMP", _sqlDesc, _sqlErr);
                            ...
                            Public.SendDatabaseReloadRequest("SBRM_ROBOT_RULE_ROUTESTEP_JUMP");

                            if (_sqlErr == string.Empty)
                            {
                                ShowMessage(this, lblCaption.Text, "", "Robot Rule Step Jump Save Success!", MessageBoxIcon.Information);
                            }
                            this.GetDataGridView();
                        }
                        catch (Exception ex)
        ...
        private void dgvCellBackColor()
        {
            ...
            try
            {
                ...
                {
                    SBRM_ROBOT_RULE_ROUTESTEP_JUMP obj = dr.DataBoundItem as SBRM_ROBOT_RULE_ROUTESTEP_JUMP;

                    if (ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_ROUTESTEP_JUMP>().Any(
                        msg => msg.ROBOTNAME == CurRobot.RobotName && msg.ROUTEID == obj.ROUTEID && msg.STEPID == obj.STEPID && msg.JUMPCONDITIONID.Equals(obj.JUMPCONDITIONID)))
                    {
                        dr.DefaultCellStyle.BackColor = Color.FromArgb(214, 233, 177);
                    }
                    else if (ctxBRM.GetChangeSet().Deletes.OfType<SBRM_ROBOT_RULE_ROUTESTEP_JUMP>().Any(
                        msg => msg.ROBOTNAME == CurRobot.RobotName && msg.ROUTEID == obj.ROUTEID && msg.STEPID == obj.STEPID && msg.JUMPCONDITIONID.Equals(obj.JUMPCONDITIONID)))
                    {
                        dr.DefaultCellStyle.BackColor = Color.Red;
                    }
                    else if (ctxBRM.GetChangeSet().Updates.OfType<SBRM_ROBOT_RULE_ROUTESTEP_JUMP>().Any(
                        msg => msg.ROBOTNAME == CurRobot.RobotName && msg.ROUTEID == obj.ROUTEID && msg.STEPID == obj.STEPID && msg.JUMPCONDITIONID.Equals(obj.JUMPCONDITIONID)))
                    {
    ...
    public class clsRobotRuleRouteStepJumpTable : SBRM_ROBOT_RULE_ROUTESTEP_JUMP
    {
