...
namespace UniOPI
{
    public partial class FormRobot_Rule_Filter_Edit : FormBase
    {
        ...
        SBRM_ROBOT_RULE_FILTER ObjEdit = null;

        public FormRobot_Rule_Filter_Edit()
        {
            InitializeComponent();

            this.lblCaption.Text = "Robot Rule Filter Edit";

        }

        public FormRobot_Rule_Filter_Edit(string _routeID, int _stepID, Robot _robot, SBRM_ROBOT_RULE_FILTER objProcess)
            : this()
        {
            CurRouteID = _routeID;
            CurStepID = _stepID;
        ...
        {           
            try
            {
                ...
                MethodDef.Location = new Point(4, 104);
                pnlNew.Controls.Add(MethodDef);

                MethodDef.InitialRBMethod("Filter");
            ...
            try
            {
                ...
                switch (_btn.Tag.ToString())
                {
                    case "Add":
                        ...
                        if (CheckData() == false)
                        {
                            dgvData.Rows.Add(CurRobot.ServerName, txtRobotName.Text, txtRouteID.Text, txtStepID.Text, txtItemID.Text, txtItemSeq.Text, MethodDef.GetObjectName(), MethodDef.GetMethodName(), MethodDef.GetFunName(), chkIsEnable.Checked, txtDescription.Text, DateTime.Now.ToString());
                            ClearData();
                        }
                        break;
                        #endregion
                    ...
                    case "OK":
                        ...
                        switch (FormMode)
                        {
                            case FormMode.AddNew:
                                ...
                                SBRM_ROBOT_RULE_FILTER _objAdd = null;

                                foreach (DataGridViewRow _row in dgvData.Rows)
                                {
                                    _objAdd = new SBRM_ROBOT_RULE_FILTER();
                                    ...
                                    _ctxBRM.SBRM_ROBOT_RULE_FILTER.InsertOnSubmit(_objAdd);
                                }
        ...
        private bool CheckData()
        {
            try
            {
                ...
                var rowCnt = (from msg in ctxBRM.SBRM_ROBOT_RULE_FILTER 
                ...
                var ChangeSet = (ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_FILTER>().Where(msg => 
                ...
                var _findNewRow = (from d in dgvData.Rows.Cast<DataGridViewRow>().Where(d => d.Cells[colRobotName.Name].Value.ToString().Equals(CurRobot.RobotName.ToString()) && 
                                                                                             ...
                                                                                             d.Cells[colItemID.Name].Value.ToString().Equals(txtItemID.Text))
