...
namespace UniOPI
{
    public partial class FormRobot_Route_Step_ByPass_Edit : FormBase
    {
        ...
        SBRM_ROBOT_RULE_ROUTESTEP_BYPASS ObjEdit = null;

        public FormRobot_Route_Step_ByPass_Edit()
        {
            InitializeComponent();

            this.lblCaption.Text = "Route Step ByPass Edit";
        }

        public FormRobot_Route_Step_ByPass_Edit(string routeID, int stepID, Robot robot, SBRM_ROBOT_RULE_ROUTESTEP_BYPASS objProcess, List<int> lstStepID)
            : this()
        {
            CurRouteID = routeID;
            CurStepID = stepID;
        ...
        private void FormRobot_Route_Step_ByPass_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                MethodDef.InitialRBMethod("RouteStepByPass");
                ...
                if (FormMode == UniOPI.FormMode.Modify)
                {
                    ...
                    if (ObjEdit != null)
                    {
                        ...
                        txtByPassItemSeq.Text = ObjEdit.BYPASSITEMSEQ.ToString();
            ...
            try
            {
                ...
                switch (_btn.Tag.ToString())
                {
                    case "Add":
                        ...
                        if (CheckData() == false)
                        {
                            dgvData.Rows.Add(CurRobot.ServerName, txtRobotName.Text, txtRouteID.Text, txtStepID.Text, txtByPassConditionID.Text, txtDescription.Text, cboGotoStepID.SelectedItem.ToString(), MethodDef.GetObjectName(), MethodDef.GetMethodName(), MethodDef.GetFunName(), txtByPassItemSeq.Text, chkIsEnable.Checked, DateTime.Now.ToString(), txtRemarks.Text);
                            ClearData();
                        }
                        break;
                        #endregion
                    ...
                    case "OK":
                        ...
                        switch (FormMode)
                        {
                            case FormMode.AddNew:
                                ...
                                SBRM_ROBOT_RULE_ROUTESTEP_BYPASS _objAdd = null;

                                foreach (DataGridViewRow _row in dgvData.Rows)
                                {
                                    _objAdd = new SBRM_ROBOT_RULE_ROUTESTEP_BYPASS();

                                    _objAdd.OBJECTKEY = 0;
                                    _objAdd.SERVERNAME = CurRobot.ServerName;
                                    _objAdd.ROBOTNAME = _row.Cells[colRobotName.Name].Value.ToString();
                                    _objAdd.BYPASSCONDITIONID = _row.Cells[colByPassConditionID.Name].Value.ToString();
                                    ...
                                    _objAdd.BYPASSITEMSEQ = int.Parse(_row.Cells[colByPassItemSeq.Name].Value.ToString());
                                    ...
                                    _ctxBRM.SBRM_ROBOT_RULE_ROUTESTEP_BYPASS.InsertOnSubmit(_objAdd);
                                }
                            ...
                            case FormMode.Modify:
                                ...
                                ObjEdit.BYPASSITEMSEQ = int.Parse(txtByPassItemSeq.Text);
        ...
        {            
            try
            {
                ...
                {
                    if (_txt is TextBox && String.IsNullOrEmpty(_txt.Text) == true && (_txt.Name.Equals(txtByPassConditionID.Name) || (_txt.Name.Equals(txtByPassItemSeq.Name))))
                    {
                ...
                var rowCnt = (from msg in ctxBRM.SBRM_ROBOT_RULE_ROUTESTEP_BYPASS 
                              where msg.ROBOTNAME.Equals(CurRobot.RobotName) && 
                              msg.ROUTEID.Equals(CurRouteID) && 
                              msg.STEPID.Equals(CurStepID) && 
                              msg.BYPASSCONDITIONID.Equals(txtByPassConditionID.Text) select msg).Count();

                var ChangeSet = (ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_ROUTESTEP_BYPASS>().Where(d => 
                                            d.ROBOTNAME.Equals(CurRobot.RobotName) && 
                                            d.ROUTEID.Equals(CurRouteID) && 
                                            d.BYPASSCONDITIONID.Equals(txtByPassConditionID.Text))).Count();

                if (rowCnt > 0 || ChangeSet > 0)
                {
                    string errMsg = string.Format("UNIQUE KEY constraint violated. Duplicate key is  Item ID[{0}] !", txtByPassConditionID.Text);
                ...
                var _findNewRow = (from d in dgvData.Rows.Cast<DataGridViewRow>().Where(d => d.Cells[colRobotName.Name].Value.ToString().Equals(CurRobot.RobotName.ToString()) && 
                                                                                             ...
                                                                                             d.Cells[colByPassConditionID.Name].Value.ToString().Equals(txtByPassConditionID.Text))
                                   select d).FirstOrDefault();

                if (_findNewRow != null)
                {
                    string _msg = string.Format(" Item ID [{0}]  is already insert. Please confirm whether you will overwite data ?", txtByPassConditionID.Text);
                    ...
                    _findNewRow.Cells[colByPassItemSeq.Name].Value = txtByPassItemSeq.Text;
        ...
        private void ClearData()
        {
            ...
            try
            {
                txtByPassItemSeq.Text = string.Empty;
                txtByPassConditionID.Text = string.Empty;
        ...
        private void dgvData_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (e.RowIndex != -1)
                {
                    ...
                    txtByPassItemSeq.Text = dgvData.Rows[e.RowIndex].Cells[colByPassItemSeq.Name].Value.ToString();
        ...
        {
            txtByPassConditionID.Text = MethodDef.GetMethodName();
