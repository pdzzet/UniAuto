...
namespace UniOPI
{
    public partial class FormRobot_Rule_Orderby : FormBase
    {
        ...
        public FormRobot_Rule_Orderby()
        {
            InitializeComponent();

            this.lblCaption.Text = "Robot Rule Orderby";
        }

        public FormRobot_Rule_Orderby(string routeID, int stepID, Robot robot, List<int> lstStepID)
            : this()
        {
            CurRouteID = routeID;
            CurStepID = stepID;
        ...
        {            
            try
            {
                ...
                //objTables = new List<SBRM_ROBOT_RULE_ORDERBY>();
                UniBCSDataContext _ctxBRM = FormMainMDI.G_OPIAp.DBBRMCtx;

                //資料庫資料
                var _selData = from rb in _ctxBRM.SBRM_ROBOT_RULE_ORDERBY
                ...
                //已修改未更新物件
                var _addData = _ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_ORDERBY>().Where( add => 
                    add.SERVERNAME == CurRobot.ServerName &&
                    add.ROBOTNAME == CurRobot.RobotName &&
                    add.ROUTEID == CurRouteID &&
                    add.STEPID == CurStepID);

                List<SBRM_ROBOT_RULE_ORDERBY> _objTables = _selData.ToList();

                _objTables.AddRange(_addData.ToList());

                List<clsRobotRuleOrderByTable> _lstCondition = new List<clsRobotRuleOrderByTable>();

                #region Get Description & Function Key
                foreach (SBRM_ROBOT_RULE_ORDERBY _data in _objTables)
                {
                    clsRobotRuleOrderByTable _tmp = new clsRobotRuleOrderByTable();
                ...
                SortableBindingList<clsRobotRuleOrderByTable> _fieldList = new SortableBindingList<clsRobotRuleOrderByTable>(_lstCondition);
            ...
            try
            {
                ...
                switch (_btn.Tag.ToString())
                {
                    case "Add":
                        ...
                        #region Add
                        FormRobot_Rule_Orderby_Edit _frmAdd = new FormRobot_Rule_Orderby_Edit(CurRouteID, CurStepID, CurRobot, null);
                    ...
                    case "Modify":
                        ...
                        clsRobotRuleOrderByTable objSelModify = (dgvData.SelectedRows[0].DataBoundItem) as clsRobotRuleOrderByTable;

                        SBRM_ROBOT_RULE_ORDERBY objProcess = null;

                        if (objSelModify.OBJECTKEY == 0)
                        {
                            // 修改的是尚未Submit的新增 -- //UK : ServerName + Robot Name + RouteID + Step ID +  + Item ID     
                            var objAddModify = (from d in _ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_ORDERBY>()
                        ...
                        {
                            // 修改的是尚未Submit的新增
                            var objModify = (from d in _ctxBRM.SBRM_ROBOT_RULE_ORDERBY
                        ...
                        FormRobot_Rule_Orderby_Edit _frmModify = new FormRobot_Rule_Orderby_Edit(CurRouteID, CurStepID, CurRobot, objProcess);
                    ...
                    case "Delete":
                        ...
                        clsRobotRuleOrderByTable objEach = null;

                        SBRM_ROBOT_RULE_ORDERBY objtoDelete = null;

                        foreach (DataGridViewRow selectedRow in dgvData.SelectedRows)
                        {
                            objEach = selectedRow.DataBoundItem as clsRobotRuleOrderByTable;

                            if (objEach.OBJECTKEY > 0)
                            {
                                objtoDelete = (from proc in _ctxBRM.SBRM_ROBOT_RULE_ORDERBY
                                               where proc.OBJECTKEY.Equals(objEach.OBJECTKEY)
                                               select proc).FirstOrDefault();
                            }
                            else
                            {
                                objtoDelete = (from proc in _ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_ORDERBY>()
                            ...
                            if (objtoDelete != null)
                                _ctxBRM.SBRM_ROBOT_RULE_ORDERBY.DeleteOnSubmit(objtoDelete);
                        }
                    ...
                    case "Save":
                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    if (objToInsert.GetType().Name != "SBRM_ROBOT_RULE_ORDERBY") continue;
                                    SBRM_ROBOT_RULE_ORDERBY _updateData = (SBRM_ROBOT_RULE_ORDERBY)objToInsert;
                                    _updateData.LASTUPDATETIME = _upDateTime;

                                    _sqlDesc = _sqlDesc + (_sqlDesc == string.Empty ? string.Empty : ";") + string.Format("ADD [{0},{1},{2},{3},{4},{5},{6},{7}]", _updateData.ROBOTNAME, _updateData.ROUTEID, _updateData.STEPID, _updateData.ITEMID, _updateData.ITEMSEQ,_updateData.ORDERBY, _updateData.ISENABLED, _updateData.DESCRIPTION);
                                }
                                #endregion
                                ...
                                foreach (object objToModify in _ctxBRM.GetChangeSet().Updates)
                                {
                                    ...
                                    SBRM_ROBOT_RULE_ORDERBY _updateData = (SBRM_ROBOT_RULE_ORDERBY)objToModify;
                                    _updateData.LASTUPDATETIME = _upDateTime;
                                ...
                                {
                                    if (objToDelete.GetType().Name != "SBRM_ROBOT_RULE_ORDERBY") continue;
                                    SBRM_ROBOT_RULE_ORDERBY _updateData = (SBRM_ROBOT_RULE_ORDERBY)objToDelete;
                            ...
                            string _err = UniTools.InsertOPIHistory_DB("SBRM_ROBOT_RULE_ORDERBY", _sqlDesc, _sqlErr);
                            ...
                            Public.SendDatabaseReloadRequest("SBRM_ROBOT_RULE_ORDERBY");
        ...
        {            
            try
            {
                ...
                {
                    SBRM_ROBOT_RULE_ORDERBY obj = dr.DataBoundItem as SBRM_ROBOT_RULE_ORDERBY;

                    if (ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_RULE_ORDERBY>().Any(
                    ...
                    else if (ctxBRM.GetChangeSet().Deletes.OfType<SBRM_ROBOT_RULE_ORDERBY>().Any(
                        msg => msg.ROBOTNAME == CurRobot.RobotName && msg.ROUTEID == obj.ROUTEID && msg.STEPID == obj.STEPID && msg.ITEMID.Equals(obj.ITEMID)))
                    {
                        dr.DefaultCellStyle.BackColor = Color.Red;
                    }
                    else if (ctxBRM.GetChangeSet().Updates.OfType<SBRM_ROBOT_RULE_ORDERBY>().Any(
                        msg => msg.ROBOTNAME == CurRobot.RobotName && msg.ROUTEID == obj.ROUTEID && msg.STEPID == obj.STEPID && msg.ITEMID.Equals(obj.ITEMID)))
                    {
    ...
    public class clsRobotRuleOrderByTable : SBRM_ROBOT_RULE_ORDERBY
    {
