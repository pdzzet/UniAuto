...
namespace UniOPI
{
    public partial class FormRobot_Poc_Result_Handle : FormBase
    {
        string CurRouteID = string.Empty;
        int CurStepID = 0;//現在step值
        Robot CurRobot = null; //Robot資訊

        //List<SBRM_ROBOT_PROC_RESULT_HANDLE> objTables = null;

        List<int> LstStepID = null;//記錄各Step的數值

        public FormRobot_Poc_Result_Handle()
        {
            InitializeComponent();

            this.lblCaption.Text = "Robot Poc Result Handle";
        }

        public FormRobot_Poc_Result_Handle(string routeID, int stepID, Robot robot, List<int> lstStepID)
            : this()
        {
            CurRouteID = routeID;
            CurStepID = stepID;
        ...
        {            
            try
            {
                ...
                //資料庫資料
                var _selData = from rb in _ctxBRM.SBRM_ROBOT_PROC_RESULT_HANDLE
                ...
                //已修改未更新物件
                var _addData = _ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_PROC_RESULT_HANDLE>().Where( add => 
                    add.SERVERNAME == CurRobot.ServerName &&
                    add.ROBOTNAME == CurRobot.RobotName &&
                    add.ROUTEID == CurRouteID &&
                    add.STEPID == CurStepID);

                List<SBRM_ROBOT_PROC_RESULT_HANDLE> _objTables = _selData.ToList();

                _objTables.AddRange(_addData.ToList());

                List<clsRobotProcResultHandleTable> _lstCondition = new List<clsRobotProcResultHandleTable>();

                #region Get Description & Function Key
                foreach (SBRM_ROBOT_PROC_RESULT_HANDLE _data in _objTables)
                {
                    clsRobotProcResultHandleTable _tmp = new clsRobotProcResultHandleTable();
                ...
                SortableBindingList<clsRobotProcResultHandleTable> _fieldList = new SortableBindingList<clsRobotProcResultHandleTable>(_lstCondition);
            ...
            try
            {
                ...
                switch (_btn.Tag.ToString())
                {
                    case "Add":
                        ...
                        #region Add
                        FormRobot_Poc_Result_Handle_Edit _frmAdd = new FormRobot_Poc_Result_Handle_Edit(
                    ...
                    case "Modify":
                        ...
                        clsRobotProcResultHandleTable objSelModify = (dgvData.SelectedRows[0].DataBoundItem) as clsRobotProcResultHandleTable;

                        SBRM_ROBOT_PROC_RESULT_HANDLE objProcess = null;

                        if (objSelModify.OBJECTKEY == 0)
                        {
                            // 修改的是尚未Submit的新增 -- //UK : ServerName + Robot Name + RouteID + Step ID +  + Item ID     
                            var objAddModify = (from d in _ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_PROC_RESULT_HANDLE>()
                                                where d.SERVERNAME == objSelModify.SERVERNAME && 
                                                      d.ROBOTNAME == objSelModify.ROBOTNAME && 
                                                      d.ROUTEID == objSelModify.ROUTEID && 
                                                      d.STEPID == objSelModify.STEPID && 
                        ...
                        {
                            // 修改的是尚未Submit的新增
                            var objModify = (from d in _ctxBRM.SBRM_ROBOT_PROC_RESULT_HANDLE
                        ...
                        FormRobot_Poc_Result_Handle_Edit _frmModify = new FormRobot_Poc_Result_Handle_Edit(
                    ...
                    case "Delete":
                        ...
                        clsRobotProcResultHandleTable objEach = null;
                        SBRM_ROBOT_PROC_RESULT_HANDLE objtoDelete = null;

                        foreach (DataGridViewRow selectedRow in dgvData.SelectedRows)
                        {
                            objEach = selectedRow.DataBoundItem as clsRobotProcResultHandleTable;

                            if (objEach.OBJECTKEY > 0)
                            {
                                objtoDelete = (from proc in _ctxBRM.SBRM_ROBOT_PROC_RESULT_HANDLE
                                               where proc.OBJECTKEY.Equals(objEach.OBJECTKEY)
                                               select proc).FirstOrDefault();
                            }
                            else
                            {
                                objtoDelete = (from proc in _ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_PROC_RESULT_HANDLE>()
                            ...
                            if (objtoDelete != null)
                                _ctxBRM.SBRM_ROBOT_PROC_RESULT_HANDLE.DeleteOnSubmit(objtoDelete);
                        }
                    ...
                    case "Save":
                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    if (objToInsert.GetType().Name != "SBRM_ROBOT_PROC_RESULT_HANDLE") continue;
                                    SBRM_ROBOT_PROC_RESULT_HANDLE _updateData = (SBRM_ROBOT_PROC_RESULT_HANDLE)objToInsert;
                                ...
                                foreach (object objToModify in _ctxBRM.GetChangeSet().Updates)
                                {
                                    ...
                                    SBRM_ROBOT_PROC_RESULT_HANDLE _updateData = (SBRM_ROBOT_PROC_RESULT_HANDLE)objToModify;
                                    _updateData.LASTUPDATETIME = _upDateTime;
                                ...
                                {
                                    if (objToDelete.GetType().Name != "SBRM_ROBOT_PROC_RESULT_HANDLE") continue;
                                    SBRM_ROBOT_PROC_RESULT_HANDLE _updateData = (SBRM_ROBOT_PROC_RESULT_HANDLE)objToDelete;
                            ...
                            string _err = UniTools.InsertOPIHistory_DB("SBRM_ROBOT_PROC_RESULT_HANDLE", _sqlDesc, _sqlErr);
                            ...
                            Public.SendDatabaseReloadRequest("SBRM_ROBOT_PROC_RESULT_HANDLE");

                            if (_sqlErr == string.Empty)
                            {
                                ShowMessage(this, lblCaption.Text, "", "Robot Proc Result Handle Save Success!", MessageBoxIcon.Information);
                            }
                            this.GetDataGridView();
                        }
                        catch (Exception ex)
        ...
        private void dgvCellBackColor()
        {
            try
            {
                ...
                {
                    SBRM_ROBOT_PROC_RESULT_HANDLE obj = dr.DataBoundItem as SBRM_ROBOT_PROC_RESULT_HANDLE;

                    if (ctxBRM.GetChangeSet().Inserts.OfType<SBRM_ROBOT_PROC_RESULT_HANDLE>().Any(
                    ...
                    else if (ctxBRM.GetChangeSet().Deletes.OfType<SBRM_ROBOT_PROC_RESULT_HANDLE>().Any(
                        msg => msg.ROBOTNAME == CurRobot.RobotName && msg.ROUTEID == obj.ROUTEID && msg.STEPID == obj.STEPID && msg.ITEMID.Equals(obj.ITEMID)))
                    {
                        dr.DefaultCellStyle.BackColor = Color.Red;
                    }
                    else if (ctxBRM.GetChangeSet().Updates.OfType<SBRM_ROBOT_PROC_RESULT_HANDLE>().Any(
                        msg => msg.ROBOTNAME == CurRobot.RobotName && msg.ROUTEID == obj.ROUTEID && msg.STEPID == obj.STEPID && msg.ITEMID.Equals(obj.ITEMID)))
                    {
    ...
    public class clsRobotProcResultHandleTable : SBRM_ROBOT_PROC_RESULT_HANDLE
    {
